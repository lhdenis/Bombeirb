...
SDL_Surface* bonus_bomb[4];

static void bonus_bomb_load() {
	bonus_bomb[BONUS_BOMB_RANGE_INC] = image_load(IMG_BONUS_BOMB_RANGE_INC);
	bonus_bomb[BONUS_BOMB_RANGE_DEC] = image_load(IMG_BONUS_BOMB_RANGE_DEC);
	bonus_bomb[BONUS_BOMB_NB_INC] = image_load(IMG_BONUS_BOMB_NB_INC);
	bonus_bomb[BONUS_BOMB_NB_DEC] = image_load(IMG_BONUS_BOMB_NB_DEC);	
}

static void bonus_bomb_unload() {
	for (int i = 0; i < 4; i++)
		SDL_FreeSurface(bonus_bomb[i]);
}

SDL_Surface* sprite_get_bonus_bomb(enum bonus_type bonus_type) {
	assert(bonus_bomb[bonus_type]);
	return bonus_bomb[bonus_type];
}

#define IMG_BONUS_LIFE  "sprite/banner_life.png"

SDL_Surface* bonus_life;

static void bonus_life_load() {
	bonus_life = image_load(IMG_BONUS_LIFE);
}

static void bonus_life_unload() {
	SDL_FreeSurface(bonus_life);
}
