...
void drop_bomb(struct game *game, struct map *map){
    ...
    (&tab_bombs[stock_bombs])->dropped = 1;

    int x = player_get_x(player);
    int y = player_get_y(player);

    (&tab_bombs[stock_bombs])->x = x;
    (&tab_bombs[stock_bombs])->y = y;
...
void bomb_update(struct map* map, struct player* player){ //update all the bombs
    ...
    for (int i = 0; i < 9; i++){
		//printf("%d %d %d", (&tab_bombs[0])->bomb_timer, (&tab_bombs[1])->bomb_timer, (&tab_bombs[2])->bomb_timer);
...
void explosion_update(struct map* map){ //update all the bombs
    ...
    struct bomb *tab_bomb = map_get_tab_bombs(map);
    ...
    for (int i = 0; i < 9; i++){
        ...
        if ((&tab_bomb[i])->dropped == 1 && (&tab_bomb[i])->bomb_timer < 0){
            ...
            if((&tab_explosion[i])->timer != 0)
                explosion_update_aux(map, &tab_explosion[i]);
		}
        if((&tab_explosion[i])->timer == 0){
